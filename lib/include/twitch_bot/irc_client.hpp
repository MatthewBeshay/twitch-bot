...
public:
    ...
    boost::asio::awaitable<void> connect(std::span<const std::string_view> channels) noexcept;

    /// Append CRLF and send an IRC message.
    boost::asio::awaitable<void> send_line(std::string_view message) noexcept;
    ...
    template <typename Handler> boost::asio::awaitable<void> read_loop(Handler &&handler) noexcept;

    /// Periodic PING every four minutes until closed.
    boost::asio::awaitable<void, boost::asio::any_io_executor> ping_loop() noexcept;
...
boost::asio::awaitable<void> IrcClient::read_loop(Handler &&handler) noexcept
{
    for (;;) {
