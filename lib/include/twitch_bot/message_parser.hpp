...
/**
 * @brief A minimal parsed IRC message. All fields are string_views into the original rawLine.
 *
 * @note The caller must guarantee that rawLine outlives the returned IrcMessage.
 */
struct IrcMessage
{
    TagMap           tags;     ///< IRC tags (each key/value is a string_view into rawLine)
    std::string_view prefix;   ///< Optional prefix (e.g. "foo!foo@foo.tmi.twitch.tv"); empty if none
...
/**
 * @brief Parse exactly one raw IRC line (without the final CRLF) into an IrcMessage.
 *
 ...
 * @return IrcMessage  Parsed tags, prefix, command, params, and trailing content.
 */
