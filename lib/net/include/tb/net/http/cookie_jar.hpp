...
public:
  ...
  void evict_expired(std::chrono::system_clock::time_point now =
                         std::chrono::system_clock::now()) {
    purge_expired(now);
  }
  void clear() noexcept { by_domain_.clear(); }

private:
  // domain -> bag of cookies (exact-domain matching)
  std::unordered_map<std::string, std::vector<Cookie>> by_domain_;

  static bool path_match(std::string_view req_path,
                         std::string_view cookie_path) noexcept;

  static bool domain_match(std::string_view host,
                           std::string_view cookie_domain) noexcept;

  static void upsert(std::vector<Cookie>& vec, Cookie&& c);
  static void erase_exact(std::vector<Cookie>& vec,
