...
namespace http_client
{
    ...
    namespace detail
    {
        ...
        inline std::string
        host_header_value(std::string_view host, std::string_view port, std::string_view scheme)
        {
    ...
    public:
        ...
        void enable_cookies(bool on = true) noexcept
        {
            cookies_enabled_ = on;
        }
        void clear_cookies() noexcept
        {
            cookies_.clear();
        }
        ...
        void set_redirect_policy(tb::net::RedirectPolicy p) noexcept
        {
            redirect_policy_ = p;
        }
        [[nodiscard]] tb::net::RedirectPolicy redirect_policy() const noexcept
        {
            return redirect_policy_;
        }
    ...
    private:
        ...
        tb::net::RedirectPolicy redirect_policy_{};
                                                    ...
                                                    ChunkHandler handler)
    {
        ...
        auto [it, inserted] = pool_.try_emplace(key, std::pmr::vector<std::shared_ptr<connection>>(&handler_buffer_));
        auto& vec = it->second;
        if (inserted)
        {
        ...
        if (auto itCE = resp.base().find(http::field::content_encoding); itCE != resp.base().end())
        {
        ...
        if (cookies_enabled_)
        {
            ...
            for (const auto& f : resp.base())
            {
                ...
                {
                    cookies_.store_from_set_cookie(std::string_view{ f.value().data(), f.value().size() },
                                                   host,
                                                   path,
                                                   /*from_https*/ true);
                }
        ...
        if (is_chunked)
        {
            ...
            for (;;)
            {
                ...
                beast::get_lowest_layer(conn->stream).expires_after(k_http_read_timeout);
                const std::size_t n = co_await conn->stream.async_read_some(conn->buffer.prepare(to_read), tok);
        ...
        else
        {
            ...
            {
                auto path = detail::path_from_target(target);
                for (const auto& f : full.base())
                {
                    if (f.name() == http::field::set_cookie)
                    {
                        cookies_.store_from_set_cookie(
                            std::string_view{ f.value().data(), f.value().size() }, host, path,
                            /*from_https*/ true);
                    }
                }
                cookies_.evict_expired();
