...
constexpr bool any(enc v) { return static_cast<unsigned>(v) != 0; }
...
[[nodiscard]] inline bool decode(std::string_view in,
                                 enc which,
                                 std::string& out,
                                 std::error_code& ec) {
    if (which == enc::none) {
...
// ---- inline helpers + definition of parse_content_encoding ----
...
inline enc parse_content_encoding(std::string_view value) {
    ...
    while (!value.empty()) {
        ...
        std::string_view token = (comma == std::string_view::npos) ? value
        ...
        if (t == "identity")      { /* no-op */ }
        else if (t == "gzip")     result = result | enc::gzip;
        else if (t == "x-gzip")   result = result | enc::gzip;
        else if (t == "br")       result = result | enc::br;
        else if (t == "deflate")  result = result | enc::deflate;
        else { /* unknown -> ignore */ }
    }
    return result;
}
