...
Abstract:
- Parse and apply HTTP Content-Encoding values.
- Liberal parser returns a bitmask of recognised codings; decode applies one step.
- gzip and br backends are defined elsewhere; deflate is reserved.
*/
#pragma once
...
namespace tb::net::encoding
{
    ...
    // Bitmask of recognised encodings.
    ...
    // Lowercase ASCII for case-insensitive matching of header tokens.
    ...
    inline enc parse_content_encoding(std::string_view value)
    {
        ...
        while (!value.empty())
        {
            ...
            else if (t == "gzip" || t == "x-gzip") // accept legacy spelling
            {
