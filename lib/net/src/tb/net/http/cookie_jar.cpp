...
bool CookieJar::path_match(std::string_view req_path,
  ...
  if (req_path.substr(0, cookie_path.size()) != cookie_path) return false;
  return (req_path.size() == cookie_path.size()) ||
         (cookie_path.back() == '/' || req_path[cookie_path.size()] == '/');
}

bool CookieJar::domain_match(std::string_view host,
...
void CookieJar::upsert(std::vector<Cookie>& vec, Cookie&& c) {
  auto it = std::find_if(vec.begin(), vec.end(), [&](const Cookie& x) {
    return x.name == c.name && x.path == c.path;
  });
  if (it != vec.end()) {
    *it = std::move(c);
  } else {
...
void CookieJar::erase_exact(std::vector<Cookie>& vec,
                            std::string_view name,
                            std::string_view path) {
  vec.erase(std::remove_if(vec.begin(),
                           vec.end(),
                           [&](const Cookie& x) {
                             return x.name == name && x.path == path;
                           }),
            vec.end());
}

Cookie CookieJar::normalize(Cookie c,
  ...
  // We could also default 'secure' from from_https, but typically the Set-Cookie parser sets it.
  return c;
}
...
void CookieJar::store(Cookie&& c,
                      ...
                      bool from_https,
                      std::chrono::system_clock::time_point now) {
  Cookie nc = normalize(std::move(c), default_domain, default_path, from_https);
  if (nc.expired_at(now)) {
    auto it = by_domain_.find(nc.domain);
    if (it != by_domain_.end()) {
      erase_exact(it->second, nc.name, nc.path);
...
void CookieJar::store_from_set_cookie(std::string_view set_cookie_line,
  ...
  // If server asked to delete (Max-Age <= 0 or Expires <= now), remove existing
  if (c.expired_at(now)) {
...
std::vector<Cookie> CookieJar::matching(std::string_view host,
  ...
  if (it == by_domain_.end()) return out;
  ...
  for (const auto& c : bag) {
    ...
    if (!domain_match(host, c.domain)) continue;
...
void CookieJar::purge_expired(std::chrono::system_clock::time_point now) {
  for (auto it = by_domain_.begin(); it != by_domain_.end(); ) {
    ...
    bag.erase(std::remove_if(bag.begin(), bag.end(),
                             [&](const Cookie& c) { return c.expired_at(now); }),
              bag.end());
    if (bag.empty()) it = by_domain_.erase(it);
    else ++it;
  }
}
