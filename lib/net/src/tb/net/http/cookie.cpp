...
namespace tb::net
{
    ...
    namespace
    {
        ...
        {
            while (!sv.empty() && static_cast<unsigned char>(sv.back()) <= 0x20)
            {
                sv.remove_suffix(1);
            }
        }
                                           ...
                                           bool /*from_https*/)
    {
        ...
        for (size_t i = 1; i < parts.size(); ++i)
        {
            ...
            std::string_view k = trim(eqpos == std::string_view::npos ? attr : attr.substr(0, eqpos));
            std::string_view v = eqpos == std::string_view::npos ? std::string_view{} : trim(attr.substr(eqpos + 1));

            if (ieq(k, "expires"))
            {
                if (auto tp = parse_http_date(v))
                {
                    c.expires = *tp;
                }
            }
    ...
    std::string build_cookie_header(std::span<const Cookie> cookies)
    {
        ...
        for (auto& c : cookies)
        {
            est += c.name.size() + 1 + c.value.size() + 2;
        }
