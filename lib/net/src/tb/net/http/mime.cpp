    ...
    std::optional<media_type> parse(std::string_view ct, std::error_code& ec)
    {
        ...
        if (semi != std::string_view::npos)
        {
            ...
            while (!params.empty())
            {
                ...
                std::string_view kv = (next_semi == std::string_view::npos) ? params : params.substr(0, next_semi);
                params = (next_semi == std::string_view::npos) ? std::string_view{} : params.substr(next_semi + 1);
