...
inline std::string to_lower(std::string_view sv) {
    std::string s;
    s.reserve(sv.size());
    for (unsigned char c : sv) s.push_back(static_cast<char>(std::tolower(c)));
    return s;
}
} // namespace
...
std::optional<media_type> parse(std::string_view ct, std::error_code& ec) {
    ec = {};
    trim(ct);
    if (ct.empty()) { ec = errc::invalid_content_type; return std::nullopt; }

    // split type / subtype
    auto slash = ct.find('/');
    if (slash == std::string_view::npos) { ec = errc::invalid_content_type; return std::nullopt; }
    ...
    if (semi != std::string_view::npos) {
        ...
        while (!params.empty()) {
            ...
            std::string_view kv = (next_semi == std::string_view::npos)
                                    ? params
                                    : params.substr(0, next_semi);
            params = (next_semi == std::string_view::npos)
                        ? std::string_view{}
