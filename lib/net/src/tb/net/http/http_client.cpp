...
namespace http_client
{
                         ...
                         RequestMetrics* out_metrics) -> boost::asio::awaitable<result>
    {
        ...
        std::string cur_port{ port.empty() ? std::string(detail::default_port_for_scheme("https")) : std::string(port) };
        ...
        for (std::size_t hop = 0; hop <= redirect_policy_.max_hops(); ++hop)
        {
            ...
            const std::string host_hdr = detail::host_header_value(cur_host, cur_port, /*scheme*/ "https");
            ...
            if (tb::net::is_redirect_status(status))
            {
                ...
                cur_port = to.port.empty() ? std::string(detail::default_port_for_scheme("https")) : std::move(to.port);
    ...
    {
        co_return co_await perform(boost::beast::http::verb::get, host, port, target, {}, headers, nullptr, nullptr);
    }
    ...
    auto client::post(std::string_view host,
                      ...
                      http_headers headers) -> boost::asio::awaitable<result>
    {
        co_return co_await perform(boost::beast::http::verb::post, host, port, target, body, headers, nullptr, nullptr);
    }
    ...
    {
        co_return co_await perform(boost::beast::http::verb::get, host, port, target, {}, headers, &opts, nullptr);
    }
    ...
    auto client::post_with_opts(std::string_view host,
                                ...
                                const RequestOptions& opts) -> boost::asio::awaitable<result>
    {
        co_return co_await perform(boost::beast::http::verb::post, host, port, target, body, headers, &opts, nullptr);
    }
