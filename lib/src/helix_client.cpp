...
asio::awaitable<void> HelixClient::ensureToken()
{
    ...
    // If there is a non-empty token that has not yet expired, do nothing.
...
{
    // Ensure there is a valid OAuth token.
    ...
    std::string target = std::string("/helix/streams?user_login=") + std::string(channel);
    ...
    try {
        ...
        // Network or TLS error -> treat as offline
        co_return std::nullopt;
    } catch (const std::runtime_error&) {
    ...
    try {
        ...
        auto& data_arr = root_obj.at("data");
        if (data_arr.kind() != json::kind::array || data_arr.as_array().empty()) {
            // Channel is offline
            co_return std::nullopt;
        }

        // Take first element and parse "started_at" (ISO 8601, UTC).
        auto& first_obj = data_arr.as_array()[0].as_object();
        std::string started_at = std::string(first_obj.at("started_at").as_string());
