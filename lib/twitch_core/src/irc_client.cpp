...
auto IrcClient::connect(std::span<const std::string_view> channels) -> boost::asio::awaitable<void>
{
    ...
    if (SSL* s = ssl.native_handle()) {
        if (SSL_CTX* ctx = ::SSL_get_SSL_CTX(s)) {
            if (::SSL_CTX_load_verify_locations(ctx, TB_CACERT_PEM_PATH, nullptr) != 1) {
                ...
                throw std::system_error{static_cast<int>(e), boost::asio::error::get_ssl_category(),
    ...
    static constexpr sv CAPS
        = "CAP REQ :twitch.tv/membership twitch.tv/tags twitch.tv/commands\r\n";

    // PASS, NICK, CAP.
    {
        std::array<const_buffer, 3> bufs_pass{buffer(PASS_), buffer(access_token_),
                                              boost::asio::buffer(kCRLF)};
        co_await send_buffers(bufs_pass);
    }
    {
        std::array<const_buffer, 3> bufs_nick{buffer(NICK_), buffer(control_channel_),
                                              boost::asio::buffer(kCRLF)};
...
auto IrcClient::join(std::string_view channel) noexcept -> boost::asio::awaitable<void>
{
    ...
    std::array<const_buffer, 3> bufs{buffer(JOIN_HASH), buffer(channel),
                                     boost::asio::buffer(kCRLF)};
    co_await send_buffers(bufs);
}
...
auto IrcClient::part(std::string_view channel) noexcept -> boost::asio::awaitable<void>
{
    ...
    std::array<const_buffer, 3> bufs{buffer(PART_HASH), buffer(channel),
                                     boost::asio::buffer(kCRLF)};
    co_await send_buffers(bufs);
}
    ...
    -> boost::asio::awaitable<void>
{
    ...
    std::array<const_buffer, 5> bufs{buffer(PRIVMSG_HASH), buffer(channel), buffer(SPACE_COLON),
                      ...
                      std::string_view text) noexcept -> boost::asio::awaitable<void>
{
    ...
    std::array<const_buffer, 7> bufs{
        buffer(REPLY_TAG),   buffer(parent_msg_id), buffer(SPACE_PRIV),        buffer(channel),
        buffer(SPACE_COLON), buffer(text),          boost::asio::buffer(kCRLF)};
    co_await send_buffers(bufs);
}
    ...
    -> boost::asio::awaitable<void>
{
    ...
    auto has_crlf
        = [](std::string_view v) { return v.find_first_of("\r\n") != std::string_view::npos; };
    ...
    while (pos < text.size()) {
        ...
        std::array<const_buffer, 5> bufs{buffer(PRIVMSG_HASH), buffer(channel), buffer(SPACE_COLON),
                                         buffer(cleaned), boost::asio::buffer(kCRLF)};
        co_await send_buffers(bufs);
                           ...
                           std::string_view text) noexcept -> boost::asio::awaitable<void>
{
    ...
    while (pos < text.size()) {
        ...
        std::array<const_buffer, 7> bufs{
            buffer(REPLY_TAG),   buffer(parent_msg_id), buffer(SPACE_PRIV),        buffer(channel),
            buffer(SPACE_COLON), buffer(cleaned),       boost::asio::buffer(kCRLF)};
        co_await send_buffers(bufs);
